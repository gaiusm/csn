...
BEGIN
   ...
   THEN
      ...
      END
   ELSE
      Halt (__FILE__, __LINE__, __FUNCTION__, 'symbol already stored')
   END
END PutSymKey ;
...
BEGIN
   ...
   ELSE
      Halt (__FILE__, __LINE__, __FUNCTION__,
            'trying to delete a symbol that is not in the tree - the compiler never expects this to occur')
   END
END DelSymKey ;
...
BEGIN
   ...
   IF t=NIL
   THEN
      Halt (__FILE__, __LINE__, __FUNCTION__, 'parameter t should never be NIL')
   END ;
   child := t^.Left ;
   IF child#NIL
   THEN
